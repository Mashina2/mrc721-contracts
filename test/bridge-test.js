...
contract("MuonBridge", (accounts) => {
  ...
  describe("Test add token", async () => {
    ...
    it("should not be able to add bridge token without correct data", async () => {
      ...
      expect.error(
        bridge2.addBridgeToken("125", token.name, token.symbol, token.decimals, `0x${cid.substr(1)}`, sigs, {
          from: owner,
        }),
        "!verified"
      )
      expect.error(
        bridge2.addBridgeToken(tokenId, "incorrect name", token.symbol, token.decimals, `0x${cid.substr(1)}`, sigs, {
          from: owner,
        }),
        "!verified"
      )
      expect.error(
        bridge2.addBridgeToken(tokenId, token.name, "INCORRECT_SYM", token.decimals, `0x${cid.substr(1)}`, sigs, {
          from: owner,
        }),
        "!verified"
      )
      expect.error(
        bridge2.addBridgeToken(tokenId, token.name, token.symbol, "8", `0x${cid.substr(1)}`, sigs, { from: owner }),
        "!verified"
      )
    })
  ...
  describe("Test deposit and claim", async () => {
    it("should be able to deposit and claim", async () => {
      ...
      expect.eventEmitted(result, "Deposit", (ev) => {
        return ev.user == accounts[0] && ev.amount.toString() == amount.toString() && ev.toChain.eq(web3.utils.toBN(2))
      })
